                ...
                for (i = 0; i < inputs.length; i++) {
                    if ($(inputs[i]).val() == false) {
                        ...
                        if (!$(inputs[i]).closest(".select_wrapper").next(".message_false").length) {
                            $(inputs[i]).closest(".select_wrapper").after("<p class='message_false'>" + $(inputs[i]).attr("data-false-message") + "</p>");
                        } else {
                            $(inputs[i]).closest(".select_wrapper").next().text($(inputs[i]).attr("data-false-message"));
                        }
            ...
            phone: function (thisForm) {
                ...
                for (i = 0; i < inputs.length; i++) {
                    if ($(inputs[i]).val() == false) {
                        ...
                        $(inputs[i]).addClass("false_required");
                        if (!$(inputs[i]).next(".message_false").length) {
                            $(inputs[i]).after("<p class='message_false'>" + formOptions.numberLengthFalseMessage + "</p>");
                        } else {
                            $(inputs[i]).next().text(formOptions.numberLengthFalseMessage);
                        }
                        $(inputs[i]).next(".message_false").fadeIn();
                        end = false;
                    } else {
